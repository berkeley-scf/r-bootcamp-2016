...
## 1 ##
ddply(air, .variables = .(Dest), .fun = summarize, medianDelay = median(DepDelay, na.rm = TRUE))

## 2 ## 
ddply(air, .variables = .(Dest, Month), .fun = summarize, perc95 = quantile(DepDelay, probs = 0.95, na.rm = TRUE))
```

```{r, using_the_ideas5}
## 3 ##
air2 <- ddply(air, .variables = .(Dest), .fun = transform, numFlights = length(Dest))

## 4 ##
air2 <- ddply(air, .variables = .(Dest), .fun = transform, Hour = CRSDepTime %/% 100)
table(air2$Hour)

## 5 ##
library(reshape2)
medianDelay <- ddply(air, .variables = .(Dest, Month), .fun = summarize, medianDelay = median(DepDelay, na.rm = TRUE))
dcast(medianDelay, formula = Dest ~ Month, value.var = 'medianDelay')

## 6 ## 
air2$LongDelay <- air2$DepDelay >= 30
mod <- glm(LongDelay ~ factor(Month) + factor(DayOfWeek) + factor(Hour), family = 'binomial', data = air2)

## 7 ## 
dests <- c("ORD", "SAN", "IAH", "SEA", "JFK")
air2sub <- air2[air2$Dest %in% dests, ]
glmmod <- function(df) glm(LongDelay ~ factor(Month) + factor(DayOfWeek) + factor(Hour), family = 'binomial', data = df)
mods <- dlply(air2sub, .variables = .(Dest), .fun = glmmod)
names(mods)
summary(mods[[1]])
```
...
```{r, advanced5}
## 8 ##
air2 <- ddply(air, .variables = .(Dest), .fun = transform, Hour = CRSDepTime %/% 100, LongDelay = DepDelay >= 30)
mod <- glm(LongDelay ~ factor(Month) + factor(DayOfWeek) + factor(Hour), family = 'binomial', data = air2)
predict(mod, newdata = data.frame(Month = c(12, 4), DayOfWeek = c(5, 6), Hour = c(14, 14)), type = 'response')
xbeta <- mod$coef["(Intercept)"] + mod$coef["factor(Month)12"] + mod$coef["factor(DayOfWeek)5"] + mod$coef["factor(Hour)14"]
...
glmmod <- function(df) glm(LongDelay ~ factor(Month) + factor(DayOfWeek) + factor(Hour), family = 'binomial', data = df)
mods <- dlply(air2sub, .variables = .(Dest), .fun = glmmod)
singleMod <- glm(LongDelay ~ factor(Dest) + factor(Month) + factor(DayOfWeek) + factor(Hour), family = 'binomial', data = air2sub)

predict(mod, newdata = data.frame(Dest = rep("ORD", 2), Month = c(12, 4), DayOfWeek = c(5, 6), Hour = c(14, 14)), type = 'response')
predict(mods[["ORD"]], newdata = data.frame(Month = c(12, 4), DayOfWeek = c(5, 6), Hour = c(14, 14)), type = 'response')
```
