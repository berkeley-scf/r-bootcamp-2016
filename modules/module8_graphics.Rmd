...
medal_counts <- medals %>% group_by(Medal, Year, Country) %>% 
             summarise(count = n())
head(medal_counts)
```
...
bwplot(Year~Gold, data = medal_counts_wide, main = "Gold Medal Counts") 
  # Labels are lost when lattice automatically converts Year to a factor
# Flip the coordinates so year increases on the x-axis
bwplot(Gold~Year, data = medal_counts_wide, main = "Gold Medal Counts", 
                  horizontal = FALSE) 
...
# Bar chart
barchart(count~Year, data = usa_gold_medals, width = 4, 
...
ggplot(data = median_gold_medals[1:15, ], aes(x = Country, y = med)) + 
...
```{r, fig.cap=" ", warning=FALSE}
ggplot(data = northern_hem, aes(x = Year, y = Silver)) + 
            geom_point(aes(size = Gold, color = Country))
```
...
5) Create a trellis plot of departure delay boxplots, one per destination for this subset of destinations, 
