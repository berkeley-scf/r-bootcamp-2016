...
# dplyr and tidyr refresher - we want to count the medals of each type, by country and by year
...
gold_medal_counts <- medal_counts %>% filter(Medal == "Gold")
usa_gold_medals <- gold_medal_counts %>% 
...
```{r}
plot(gold_medal_counts$Year, gold_medal_counts$count, main = "Gold Medal Counts")
plot(usa_gold_medals$Year, usa_gold_medals$count, type = "l", main = "USA Gold Medals")
```

## Other plot types
```{r}
hist(gold_medal_counts$count)
plot(density(gold_medal_counts$count))
...
boxplot(count~Country, medal_counts %>% filter(Country %in% c("United States", "Canada", "Mexico")))
```
